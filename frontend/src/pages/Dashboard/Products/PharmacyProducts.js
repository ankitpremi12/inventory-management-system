...
const PharmacyProducts = () => {
    ...
    // Calculate stats
    const totalItems = pharmacyProducts.length;
    const inStockCount = pharmacyProducts.filter(p => parseInt(p.stock || 0) > 100).length;
    const lowStockCount = pharmacyProducts.filter(p => parseInt(p.stock || 0) > 0 && parseInt(p.stock || 0) <= 100).length;
    const outOfStockCount = pharmacyProducts.filter(p => parseInt(p.stock || 0) === 0).length;
    ...
    return (
        <section style={{ animation: 'fadeIn 0.4s ease-out' }}>
            ...
            <div className="ims-tabs-container">
                <Link to="/dashboard/products/main" className="ims-tab-btn active">Main Stock</Link>
                <Link to="/dashboard/products/supplies" className="ims-tab-btn">Supplies</Link>
            </div>
            ...
            <label htmlFor="create-new-product" className="modal cursor-pointer">
                <label className="ims-modal-box modal-box lg:w-8/12 md:w-10/12 w-11/12 max-w-4xl relative" htmlFor="">
                    ...
                    <ModalHeading modalHeading={'Create a Main Stock Product'} />
