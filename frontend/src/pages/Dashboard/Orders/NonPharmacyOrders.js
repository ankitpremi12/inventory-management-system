    ...
    const addOrder = event => {
        ...
        fetch(`${BASE}/orders/supplies`, {
            ...
            .then(() => {
                ...
                document.getElementById('create-supplies-order').checked = false;
                event.target.reset();
            })
    ...
    return (
        <section style={{ animation: 'fadeIn 0.4s ease-out' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
                <div>
                    <h1 style={{ fontSize: '1.35rem', fontWeight: 800, color: 'var(--text-primary)', margin: 0 }}>Supplies Orders</h1>
            ...
            <div className="ims-tabs-container">
                <Link to="/dashboard/orders/main"     className="ims-tab-btn">Main Stock</Link>
                <Link to="/dashboard/orders/supplies" className="ims-tab-btn active">Supplies</Link>
            </div>

            {/* Add Order Modal */}
            <input type="checkbox" id="create-supplies-order" className="modal-toggle" />
            <label htmlFor="create-supplies-order" className="modal cursor-pointer z-50">
                <label className="ims-modal-box modal-box lg:w-7/12 md:w-9/12 w-11/12 max-w-2xl relative" htmlFor="">
                    <ModalCloseButton modalId="create-supplies-order" />
                    <ModalHeading modalHeading="Create a Supplies Order" />
